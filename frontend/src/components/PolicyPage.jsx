      ...
      <section className="bg-milk">
        <div className="container-bank pt-16 md:pt-24 pb-12">
          <div className="max-w-4xl">
            ...
            <h1 className="display-xl text-navy-600 mb-6">{title}</h1>
            <p className="text-[17px] text-bone-600 leading-relaxed max-w-3xl">
              {description}
            </p>
          </div>
        </div>
            ...
            <div className="col-span-12 md:col-span-8 lg:col-span-9 max-w-3xl space-y-12">
              {sections.map((s, i) => (
                <section key={i} id={`section-${i + 1}`}>
                  <p className="font-mono text-[11px] text-orange-600 mb-3 tracking-[0.22em]">
                    § {String(i + 1).padStart(2, "0")}
                  </p>
                  <h2 className="font-display text-[24px] md:text-[28px] text-navy-600 mb-5">
                    {s.heading}
                  </h2>
                  <div className="space-y-4 text-[15px] text-bone-700 leading-relaxed">
                    {s.body.split("\n\n").map((para, j) => (
                      <p key={j}>{para}</p>
                    ))}
                  </div>
